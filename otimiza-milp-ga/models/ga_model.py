...
def executar_ga_completo(zip_path=None, ativar_bem_estar=False, thr_sel=0.9, skip_reports=False):
    ...
    print(f"Bem-estar: {'ATIVADO' if ativar_bem_estar else 'DESATIVADO'}")
    print(f"peso_hard: {PAR['PESO_HARD']:,}")
    print(f"peso_desvio_comp: {PAR['PESO_DESVIO_COMP']:,}")
    print(f"peso_sdem: {PAR['PESO_SDEM']:,}")
    print(f"peso_falta_papel: {PAR['PESO_FALTA_PAPEL']:,}")
    ...
    print(f"   Tempo: {resultado_ga['tempo_execucao']:.1f}s ({resultado_ga['tempo_execucao']/60:.1f} min)")
    print(f"   Upgrades: {len(resultado_ga['funcionarios_promovidos'])}")
    print(f"   Alocações (pares x): {len(resultado_ga['x'])}")
    if not skip_reports:
        print(f"   Arquivo alocados: {excel_path}")
    print("="*70)
...
if __name__ == "__main__":
    ...
    skip_reports = bool(os.environ.get("NO_OUTPUT", "0") == "1")
