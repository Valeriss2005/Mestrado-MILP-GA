...
def executar_modelo(zip_path, ativar_bem_estar=True, saida_prefix="LARGE_V31.zip", warm_start=None):
   ...
   # =========================
    ...
    print("="*70)
    print(f"RELATÓRIO MILP - BASE LARGE - (Bem-Estar: {'ATIVO' if ativar_bem_estar else 'INATIVO'})")
    print("="*70)
    print(f"Status do Solver:                 {status_txt}")
    print(f"Projetos ativos (y):              {int(total_y)} / {Np}")
    print(f"Total de alocacoes (SUM_x):       {int(total_x_val)}")
    print(f"Profissionais alocados (únicos):  {int(unicos)} / {Nf}  ({100*unicos/max(1,Nf):.1f}%)")
    print(f"Média pessoas por projeto:        {media:.2f}")
    print(f"Upgrades realizados:              {upgrades_val}  ({100*upgrades_val/max(1,total_x_val):.2f}%)")
    print("-"*70)
    print(f"Custo total (folha):              R$ {custo_real:,.2f}")
    print("="*70)
...
if __name__ == "__main__":
    ...
    resultado = executar_modelo(PAR["ARQUIVO_ZIP"], ativar_bem_estar=True, saida_prefix="LARGE_V31")

    exportar_alocados_detalhado(resultado, out_xlsx=os.path.join(OUTPUT_DIR, "Alocados_detalhado_LARGE_V31_BE.xlsx"), thr_sel=0.9)


    # Gerar alocados detalhados + capturar df_alocados
    df_alocados = exportar_alocados_detalhado(
        resultado,
        out_xlsx=os.path.join(OUTPUT_DIR, "Alocados_detalhado_LARGE_V31_BE.xlsx"),
        thr_sel=0.9
    )

    # Diagnóstico de não alocados
    thr_sel = 0.9
    df_diag = diagnostico_nao_alocados(resultado, df_alocados, thr_sel=thr_sel)

    # Salvar diagnóstico em Excel
    caminho_diag_xls = os.path.join(OUTPUT_DIR, "Diagnostico_Nao_Alocados_MILP.xlsx")
    df_diag.to_excel(caminho_diag_xls, index=False)


    print(f"\nArquivo Excel de diagnóstico salvo em: {caminho_diag_xls}")
    ...
    print("\n=== RESUMO GLOBAL DE MOTIVOS (soma de pares bloqueados) ===")
    print(df_diag[cols_motivos].sum().to_frame("Qtde_Pares").to_string())

    def _motivo_principal(row):
        valores = row[cols_motivos].astype(float).values
        if valores.max() <= 0:
            return "Nenhum_motivo_explícito"
        idx = valores.argmax()
        return cols_motivos[idx]

    df_diag["Motivo_Principal"] = df_diag.apply(_motivo_principal, axis=1)

    resumo_motivo_principal = (
        df_diag["Motivo_Principal"]
        .value_counts()
        .rename_axis("Motivo_Principal")
        .reset_index(name="Qtde_Func")
    )

    print("\n=== MOTIVO PRINCIPAL POR FUNCIONÁRIO (contagem) ===")
    print(resumo_motivo_principal.to_string(index=False))

    # Salva aba extra
    with pd.ExcelWriter(caminho_diag_xls, engine="openpyxl", mode="a", if_sheet_exists="replace") as w:
        df_diag.to_excel(w, sheet_name="Nao_Alocados", index=False)
        resumo_motivo_principal.to_excel(w, sheet_name="Resumo_Motivos", index=False)
    print("\nAbas 'Nao_Alocados' e 'Resumo_Motivos' gravadas em:", caminho_diag_xls)
